<?php
session_start();
require "../../../../config/database.php";
require "../../../../includes/functions.php";
require "../../../../includes/constants.php";
require "../../../../filters/auth.filter.php";

if(isset($_POST['_noteRestaure']) && is_numeric($_POST['_noteRestaure'])) {

    extract($_POST);

    $_nid = $_noteRestaure;

    $note = find_one_id_in_table($_nid, 'id', 'notes');

    $board = find_table_by_id($note->b_id);

    $story_msg = "a restauré cette note: <u>".$note->note."</u>";

    $i = 1;
    $counter = $board->contributions+$i;

    $q = $db->prepare("UPDATE notes SET archivate=:archivate WHERE id=:id AND u_id=:u_id");

    set_contributions($note->b_id, 'boards', $counter, get_session('user_id'));

    set_story_msg('', $story_msg, 'activities_story', get_session('user_id'), $note->b_id);

    $q->execute([
        'archivate' => '0',
        'id' => $_nid,
        'u_id' => get_session('user_id'),
    ]);

    $q->closeCursor();

    if($q) {

        die("Reataured");

    }

}