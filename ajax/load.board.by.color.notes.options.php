...
?>
<?php if(isset($_POST['exocolor']) && isset($_POST['bid'])): ?>
    <?php
    extract($_POST);
    ...
    <?php if($listes): ?>
        <?php foreach($listes as $liste): ?>
            <?php
            $nCount = cell_count('notes', 'bl_id', $liste->id);
            $conditions = '';
            $i = 0; //Sers à savoir si c'est ton premier passage dans la boucle
            foreach($_POST['exocolor'] as $color) {
                if ($i == 0) {
                    $conditions .= "background LIKE '%".$color."%'";
                    $i++;
                } else {
                    $conditions .= " OR background LIKE '%".$color."%'";
                }
            }
                    ...
                    <div class="b-list-content board-list-content" id="js-list-note-container">
                        <div class="b-list-header u-clearfix" id="js-head-list">
                            <div class="b-list-head" id="js-list-header">
                                ...
                                <?php endif; ?>
                                <span class="blocked td-color-grey text-size-1x align-right">[<?= $nCount > 1 ? $nCount." notes" : $nCount." note" ; ?>]</span>
                            </div>
                        </div>
                        <?php if($notes): ?>
