...
<?php if(isset($_POST['note_color']) &&
    ...
    $upColor = e($note_color);

    $note = find_one_id_in_table($note_id, 'id', 'notes');

    $story_msg = "a changé la couleur de fond d'une note: <u>".$note->note."</u>.";

    $i = 1;
    $counter = $board->contributions+$i;

    $q = $db->prepare("UPDATE notes SET background = :background WHERE id = :id AND u_id = :u_id");
