...
?>
<?php if(isset($_POST['html']) && isset($_POST['note_id']) && isset($_POST['bid']) && is_numeric($_POST['note_id']) && is_numeric($_POST['bid'])): ?>
    ...
    $story_msg = "a modifier une note en: <u>".$upNote."</u>.";

    $i = 1;
    $counter = $board->contributions+$i;

    $q = $db->prepare("UPDATE notes SET note=:note WHERE id=:id AND u_id=:u_id");
